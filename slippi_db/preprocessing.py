...
def compute_winner(game: peppi_py.Game) -> Optional[int]:
  ...
  stock_counts = {}
  for i in range(0, len(game.frames.ports)):
    stock_counts[i] = game.frames.ports[i].leader.post.stocks[-1].as_py()
...
def get_metadata(game: peppi_py.Game) -> dict:
  ...
  for player in players:
    ...
    leader = game.frames.ports[port].leader
    cs = leader.post.character.to_numpy()
    character = int(mode(cs))

    player_metas.append(dict(
        port=port,
        character=character,
        type=0 if player.type == 0 else 1,
